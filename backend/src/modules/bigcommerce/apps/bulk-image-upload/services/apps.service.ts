...
class AppService {
  ...
  public async findByFiles(params: object = {}): Promise<App[]> {

    try {
      await this.db.init();
      await this.db.appsModel.updateOne(params['queryParams'], params['querySet']);
      const apps: App[] = await this.db.appsModel.find(params['queryParams']);
      return apps;
